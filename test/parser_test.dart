...
void main() {
  group('Parse task positive tests => ', () {
    ...
    test('parse task with only title (dash)', () {
      parser.BasicTask expected = parser.Task(parser.states.dash, "foo");
      expect(parser.parseTask("-foo"), expected);
    });

    test('parse task with only title (box)', () {
      parser.BasicTask expected = parser.Task(parser.states.box, "foo");
      expect(parser.parseTask("[ ]foo"), expected);
    });

    test('parse task with only title (checked box)', () {
      parser.BasicTask expected = parser.Task(parser.states.checked_box, "foo");
